        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/tushujieyueguanlixitong/upload/tushuMuBan.xls"
                        >批量导入图书数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('tushu','导入导出')"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="tushuKucunNumber"
                                      header-align="center"
                                      label="图书数量">
                        <template slot-scope="scope">
                            {{scope.row.tushuKucunNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="tushuClicknum"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('tushu','修改')" type="primary" icon="el-icon-tickets" size="mini" @click="shangxia(scope.row.id,scope.row.shangxiaTypes)">{{scope.row.shangxiaTypes == 1?'下架':'上架'}}</el-button>
                            <!--<el-button v-if="isAuth('tushu','修改')" type="primary" icon="el-icon-add-location" size="mini" @click="plusTushuKucunNumber(scope.row.id,scope.row.tushuKucunNumber)">增加图书数量</el-button>-->
                            <!--<el-button v-if="isAuth('tushu','修改')" type="primary" icon="el-icon-delete-location" size="mini" @click="reduceTushuKucunNumber(scope.row.id,scope.row.tushuKucunNumber)">减少图书数量</el-button>-->
...
<!-- 库存 -->
		<el-dialog
                :title="tushuKucunNumberTitle"
                :visible.sync="tushuKucunNumberVisible"
                width="30%">
            <span>操作数量</span><el-input-number v-model="kucunNumber" :min="1" :max="100" label="描述文字"></el-input-number>
            <span slot="footer" class="dialog-footer">
			<el-button @click="tushuKucunNumberVisible = false">取 消</el-button>
			<el-button type="primary" @click="addTushuKucunNumber()">确 定</el-button>
		  </span>
        </el-dialog>
...
<script>
    ...
    //当前表下拉框搜索条件
        ...
        //库存
            ...
            tushuKucunNumber: 0,
            //标题
            tushuKucunNumberTitle: null,
            //模态框状态
            tushuKucunNumberVisible: false,
            };
        },
        created() {
        ...
        methods: {
            ...
            //增加图书数量
			plusTushuKucunNumber(thisId,tushuKucunNumber){
			//清空数据
				//图书数量数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				//将模态框打开
				this.tushuKucunNumberVisible = true;
				//标题
				this.tushuKucunNumberTitle = '增加图书数量'
			},
			//减少图书数量
			reduceTushuKucunNumber(thisId,tushuKucunNumber){
			//清空数据
				//图书数量数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				this.tushuKucunNumberVisible = true;
				//标题
				this.tushuKucunNumberTitle = '减少图书数量'
			},
			//提交用户操作图书数量数量
			addTushuKucunNumber(){
				var type = null
				...
				if(this.kucunNumberTypes == "+"){
					type = '增加'
					maxNumber = this.tushuKucunNumber + this.kucunNumber
				}else{
					type = '减少'
					if(this.tushuKucunNumber < this.kucunNumber || this.kucunNumber <= 0){
						this.$message.error('图书数量不足不能减少');
						return false;
					}else{
						maxNumber = this.tushuKucunNumber - this.kucunNumber
					}
				...
				this.tushuKucunNumberVisible = false;
			},			//上下架
